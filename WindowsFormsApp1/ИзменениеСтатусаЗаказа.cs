...
{
    public partial class ИзменениеСтатусаЗаказа : Form
    {
        public ИзменениеСтатусаЗаказа()
        {
            InitializeComponent();
        }
        ...
        {
            string статусЗаказа = comboBox1.SelectedItem.ToString();
            ...
            string query = "UPDATE Заказы SET СтатусЗаказа = @статусЗаказа WHERE НомерКомнаты = @номерКомнаты";
        ...
        private void ИзменениеСтатусаЗаказа_MouseDown(object sender, MouseEventArgs e)
        {
        ...
        private void ИзменениеСтатусаЗаказа_MouseMove(object sender, MouseEventArgs e)
        {
            if (isDragging)
            {
        ...
        private void ИзменениеСтатусаЗаказа_MouseUp(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                isDragging = false;
        ...
        private void ИзменениеСтатусаЗаказа_Load(object sender, EventArgs e)
        {
            this.MouseDown += ИзменениеСтатусаЗаказа_MouseDown;
            this.MouseMove += ИзменениеСтатусаЗаказа_MouseMove;
            this.MouseUp += ИзменениеСтатусаЗаказа_MouseUp;
        }
    }
}
